...
const restructurePlayerData = (data) => {
    ...
    Object.entries(data.bowler_analysis).forEach(([bowler_id, bowler]) => {
        if (!players[bowler_id]) {
            players[bowler_id] = { player_id: Number(bowler_id), name: bowler.name, role: bowler.role, team_name: bowler.team_name, batting_style: bowler.batting_style, bowling_style: bowler.bowling_style, bowling: {}, batting: {} };
        }
    ...
    Object.entries(data.batting_analysis).forEach(([batsman_id, batsman]) => {
        if (!players[batsman_id]) {
            players[batsman_id] = { player_id: Number(batsman_id), name: batsman.name, role: batsman.role, team_name: batsman.team_name, batting_style: batsman.batting_style, bowling_style: batsman.bowling_style, batting: {}, bowling: {} };
        }
...
const outputPath = path.join(__dirname, `./${match_id}_playerWiseData.js`);
const fileContent = `module.exports = ${JSON.stringify(playerWiseData, null, 2)};`;
